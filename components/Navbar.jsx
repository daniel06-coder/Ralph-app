...
const Navbar = () => {
  ...
  const slideLogic = (x) => {
    return setSlider(slider === x ? null : x);
  };
      ...
      <div className=" z-10 w-full ">
        ...
        >
          ...
          >
            ...
            <button className="pointer uppercase relative flex justify-center  ">
              The best Team
              <p className="text-xs absolute top-6  ">presents</p>
            </button>
          </div>
          <div className="flex shadow-xl  max-md:justify-center max-md:items-center md:relative overflow-hidden">
            ...
            <div className=" inset-y-0  md:absolute left-0 md:px-3 py-1 flex  md:justify-center rounded-full ">
              <BiSearchAlt className="md:relative md:top-2 size-6  " />
            </div>
            <Link
              href={"/"}
        ...
        >
          ...
          {session ? (
            <div className="relative ">
              ...
              {modal && (
                <div className="absolute z-10  w-[8rem] max-md:text-sm max-md:w-[5rem] rounded-sm  bg-red-700 text-white/80  flex flex-col px-3 py-2 max-md:py-1 max-md:gap-1 gap-2  items-center pointer">
        ...
        {list.map((items, index) => {
          return (
            <div key={index} className="relative">
              {items.subItems ? (
                // has subItems — render a button, not a Link
                <button
                  onClick={() => collapseFunc(index)}
                  className="flex flex-col pb-2 border-b py-2 lg:hover:bg-white/20 border-b-red-500/50 px-5 w-full"
                >
                  <div className="flex items-center justify-between w-full">
                    <span className="max-md:text-xl font-semibold">
                      {items.label}
                    </span>
                    {collapse.includes(index) ? items.iconTwo : items.icon}
                  </div>
                </button>
              ) : (
                // no subItems — render a Link that navigates and closes menu
                <Link
                  href={items.url}
                  onClick={() => slideLogic(3)} // close menu on navigate
                  style={{ animationDelay: `${index * 0.2}s` }}
                  className={`${slider === 1 ? "fadeIn" : ""} flex flex-col pb-2 border-b py-2 lg:hover:bg-white/20 border-b-red-500/50 px-5`}
                >
                  <span className="max-md:text-xl font-semibold">
                    {items.label}
                  </span>
              ...
              >
                {items.subItems?.map((secItems, subIndex) => (
                  <Link
                    href={secItems.url}
                    key={subIndex}
                    onClick={() => slideLogic(3)} // close menu when subitem clicked
                    className="flex md:backdrop-blur-xl flex-col px-5 md:px-2 py-2 lg:hover:bg-white/20 max-md:bg-red-100/10 max-md:my-1 py-1 w-full md:w-[8rem]"
                  >
